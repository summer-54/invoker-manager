...
use std::{collections::{HashMap, HashSet, VecDeque}, io, sync::Arc};
...
pub struct Server {
    ...
    unload_invokers_uuid: HashSet<Uuid>,
...
impl Server {
    pub fn new(address: String) -> Arc<Mutex<Self>> {
        ...
        Arc::new(Mutex::new(Self {
            ...
            testing_system: None,
            unload_invokers_uuid: HashSet::new(),
            tests_results: HashMap::new(),
        }))
    }

    pub async fn start_invokers_side(server: Arc<Mutex<Self>>) -> Result<(), String> {
        let Ok(listener) = TcpListener::bind(server.lock().await.address.clone()).await else {
    ...
    pub async fn start_testing_system_side(server: Arc<Mutex<Self>>) -> Result<(), String> {
        let testing_system = match TestingSystem::connect_to("wss://127.0.0.1:5477").await {
            Ok(ts) => ts,
            Err(error) => {
    ...
    pub async fn add_invoker(server: Arc<Mutex<Self>>, stream: WebSocketStream<TcpStream>) -> Result<JoinHandle<Result<String, String>>, String> {
